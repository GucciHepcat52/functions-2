...
// CODE HERE
const postTaxPrices = prices.map((elements) => elements * 1.07);
...
// CODE HERE
const totalPopulation = populations.reduce((acc, element) => acc + element);
...
// CODE HERE
const bobsTotal = purchases.filter((element) => element.owner === "Bob").map((element) => element.price).reduce((acc, element) => acc + element);
